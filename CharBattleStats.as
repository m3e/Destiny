package 
{
	...
	public class CharBattleStats extends MovieClip
	{
		...
		public function CharBattleStats(FamilyData:Object,_Hero:Hero)
		{
			// constructor code
		...
		private function putInInv(e:Event):void
		{
			...
			if (itemSelected != null && hero.equipArray.indexOf(itemSelected) != -1)
			{
				...
				switch (itemSelected.slot)
				{
					case "RArm" :
						RHand.removeChild(itemSelected);
						break;
		...
		public function removeDisplay():void
		{
			...
			for (var k:int=0; k<hero.equipArray.length; k++)
			{
				...
				if (hero.equipArray[k] != null)
				{
					hero.equipArray[k].parent.removeChild(hero.equipArray[k]);

					hero.equipArray[k].removeEventListener(MouseEvent.MOUSE_OVER, displayStats);
					hero.equipArray[k].removeEventListener(MouseEvent.MOUSE_OUT, undisplayStats);
				}
			}
			for (var i:int=0; i< familyData.inventory.length; i++)
			{
